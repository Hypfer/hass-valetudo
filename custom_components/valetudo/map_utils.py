...
def extract_and_parse_map(image_data: bytes) -> dict:
    """
    Parses a PNG byte array, looks for a zTXt chunk with keyword 'ValetudoMap',
    decompresses it, and applies pixel preprocessing.
    """
    ...
    if map_json:
        return preprocess_map(map_json)

    return None


def preprocess_map(data: dict) -> dict:
    """
    Expands RLE compressed pixels into full x,y coordinates.
    """
